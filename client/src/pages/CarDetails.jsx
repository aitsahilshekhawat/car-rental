...
import {
  ArrowRight, Check, ChevronLeft, ChevronRight, Fuel, Gauge, Heart, MapPin,
...
export function CarDetails() {
  ...
  return <PageShell className="details-page">
    ...
    <section className="wrap section-pad related"><div className="section-heading"><div><p className="eyebrow">Keep exploring</p><h2>You might also like</h2></div><Link className="arrow-link" to="/cars">See all cars <ArrowRight size={17} /></Link></div><div className="car-grid">{cars.filter((item) => item.id !== car.id).slice(0,3).map((item) => <CarCard car={item} key={item.id} compact />)}</div></section>
